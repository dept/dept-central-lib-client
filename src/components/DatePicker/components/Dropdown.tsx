  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <ul
          ref={ref}
          className="absolute max-h-40 min-w-fit overflow-y-auto z-10 mt-1 bg-white border border-grayscale-300 rounded shadow-lg"
        >
          {options.map((option, index) => (
            <li
              key={option.value}
              className={`px-4 py-2 cursor-pointer ${
                selectedValue === option.value ? 'bg-grayscale-200' : ''
              }`}
